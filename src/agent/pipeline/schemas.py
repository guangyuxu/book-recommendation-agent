"""Structured-output schemas for the main pipeline nodes.

Each LLM-backed pipeline node (understand, plan, clarify) returns one of these via
model.with_structured_output(...). They are the contract between stages; nodes store their
model_dump() into the matching state channel. The memory subgraph's own contracts live in
...
class Understanding(BaseModel):
    ...
    @classmethod
    def _parse_stringified_list(cls, v: Any) -> Any:
        """Coerce a JSON-string list back into a list before validation.

        The model (via forced tool-calling) intermittently returns a list-valued field as a JSON
        *string* -- e.g. mentioned_books='[{"title": ...}]' -- which otherwise fails with a
        list_type error and crashes the whole turn. Parse it back; leave anything else untouched
        so a genuinely malformed value still surfaces the normal validation error (these fields
        are load-bearing -- the request itself and the named books -- so we do NOT drop them).
        """
        if isinstance(v, str):
            try:
                return json.loads(v)
    ...
    def _parse_user_signals(cls, v: Any) -> Any:
        ...
        over that would deny the user any answer; dropping the turn's signals only skips one round
        of profile extraction. So: parse if we can, otherwise fall back to [] rather than raising.
        """
        if isinstance(v, str):
            try:
                return json.loads(v)
